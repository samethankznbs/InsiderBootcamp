...
class ShoppingCart {
    ...
    /*removeItem(productId) {
        try {
            ...
            if (product) {
                product.stock += 1; // Stoka 1 adet geri ekleniyor
            }
    
            if (item.quantity > 1) {
                item.quantity -= 1; // Eğer 1'den fazlaysa sadece 1 azalt
            } else {
                this.items.splice(itemIndex, 1); // Eğer sadece 1 varsa tamamen kaldır
            }
    ...
    applyDiscount(code) {
        ...
        if (code === 'INDIRIM10' && !this.discountApplied) {
            ...
            this.calculateTotal();
            this.updateUI();
            this.showMessage('İndirim başarıyla uygulandı!');
        } else if (code !== 'INDIRIM10') {
            this.showError('Geçersiz indirim kodu! Lütfen tekrar deneyin.');
        } else if (this.discountApplied) {
            this.showError('İndirim kodu zaten uygulanmış!');
        }
    }
    ...
    showMessage(message) {
        ...
        if (messageElement) {
            if (message === 'İndirim başarıyla uygulandı!' && messageElement.innerHTML.includes(message)) {
                return;
            }
            ...
            if (message !== 'İndirim başarıyla uygulandı!') {
                setTimeout(() => {
                    newMessage.remove();
                }, 3000);
            }
